...
compileInstr ty tm (Icmp s c t o1 o2) =  [movq ~$0 ~~(fromJust(lookup s tm)), movq ~~(compileOperand tm o1) ~%RAX, cmpq ~~(compileOperand tm o2) ~%RAX, set c ~~(fromJust(lookup s tm))]--icmp ne i64 %19, note: zero temporary. and  use set to read condition flag and set tempr
